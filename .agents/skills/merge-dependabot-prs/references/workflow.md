...
If the user explicitly requested a parallel/autonomous run, launch
up to 5 Codex subagents in parallel. Each call should use the
appropriate prompt below (library or actions).

If more than 5 work units, wait for the current wave to complete
before launching the next. If parallel agents were not explicitly
requested, evaluate work units sequentially.
...
For major version bumps, use Exa MCP when configured, or Codex web
search otherwise, to search for breaking changes:
