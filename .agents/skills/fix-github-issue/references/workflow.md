...
error messages, or domain-specific concepts. If so, use Exa MCP
when configured, or Codex web search otherwise, to search for:
...
project's AGENTS.md (or CLAUDE.md during migration) standards. Keep changes minimal and focused
...
3. **Read AGENTS.md (or CLAUDE.md during migration)** (if present at repo root or `.claude/`).
...
check prose accuracy, confirm rendering). For code changes, run a
deep review against the diff (compare working tree to the upstream
main branch). Use `codex review --base <upstream-remote>/<base>`
as the default review pass. If a specialized PR review skill or
plugin is installed, it may be used as an additional pass. Produce
a list of findings ranked by severity (P1 = blocks merge,
