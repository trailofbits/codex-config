...
### Pass A — specialized review agents, if available

If an equivalent review plugin or custom agent set is installed,
launch these review passes in parallel. Tell each agent which
files changed (from `git diff --name-only <base>...HEAD`):
...
Launch these Codex subagents in parallel with Pass A when parallel
agents were explicitly requested for the run. Otherwise, run these
passes sequentially.
...
# Build prompt file (avoids heredoc shell expansion issues)
{
  ...
  if [ -f AGENTS.md ] || [ -f CLAUDE.md ] || [ -f .claude/CLAUDE.md ]; then
    ...
    echo "---"
    cat AGENTS.md CLAUDE.md .claude/CLAUDE.md 2>/dev/null
    echo "---"
  fi
  echo ""
  echo "Diff:"
...
MCP when configured, or Codex web search otherwise, to search for
solutions rather than guessing.
